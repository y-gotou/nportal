...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-bold tracking-tight text-slate-900">議事録</h1>
      <NuxtLink
    ...
    <div v-if="minutes.length" class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="m in minutes" :key="m.slug" class="hover:bg-slate-50">
            <td class="px-4 py-3">
              <p class="font-medium text-slate-900">{{ m.title }}</p>
              <p class="mt-0.5 text-xs text-slate-500 sm:hidden">{{ formatDisplayDate(m.date) }}</p>
            </td>
