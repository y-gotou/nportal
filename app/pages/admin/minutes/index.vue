...
const { data, refresh } = await useFetch<MinutesListResponse>("/api/minutes", {
  default: () => ({ minutes: [] }),
});

const minutes = computed(() => data.value?.minutes ?? []);

useSeoMeta({ title: "議事録管理" });
</script>

<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-bold tracking-tight text-foreground">議事録</h1>
      <NuxtLink
    ...
    <div v-if="minutes.length" class="overflow-hidden rounded-xl border border-border bg-surface shadow-sm">
      <table class="min-w-full divide-y divide-border">
        ...
        <tbody class="divide-y divide-border">
          <tr v-for="m in minutes" :key="m.slug" class="hover:bg-surface-hover">
            <td class="px-4 py-3">
              <p class="font-medium text-foreground">{{ m.title }}</p>
              <p class="mt-0.5 text-xs text-muted sm:hidden">{{ formatDisplayDate(m.date) }}</p>
            </td>
