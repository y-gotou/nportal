...
test("buildChatAiMessages: 過去の生成失敗メッセージと日付を扱う", () => {
  const messages = buildChatAiMessages({
    schedule,
    messages: [
      makeMessage({ id: 1, userEmail: "ai-assistant@nportal.local", body: CHAT_AI_ERROR_BODY }),
      makeMessage({ id: 2, body: "@AI 質問です" }),
    ],
    today: "2026-07-08",
  });

  assert.ok(messages[0]?.content.includes("本日の日付は 2026-07-08 です。"));
  assert.ok(!messages[1]?.content.includes(CHAT_AI_ERROR_BODY));
  assert.ok(messages[1]?.content.includes("@AI 質問です"));
});
