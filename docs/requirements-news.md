...
- 見出し(平易な日本語)
- 箇条書き要約 **2〜5 点**。3 点に固定すると情報が欠落するため、記事の情報量に応じて増減させる
- 観点に応じた見出し(§5.3)と説明文 1〜2 文を、背景色付きの枠で表示する
- 用語注は独立した枠にまとめる。難しい語が複数あれば複数件を出してよい
- タグ
- 出典名を含むリンク(`{出典名} の元記事を読む →`)と記事公開日を隣接させる
- 👍 / 👎 ボタンと合計件数

順位バッジとカテゴリはカードに表示しない。並び順が順位を表しており、カテゴリは観点別見出しと重複するため。
...
CREATE TABLE IF NOT EXISTS news_articles (
  ...
  summary        TEXT NOT NULL DEFAULT '[]',  -- 箇条書き要約 (JSON配列)
