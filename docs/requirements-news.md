...
- Tavily API のレスポンスに含まれる本文抜粋(`api.tavily.com` のみで完結する)
...
| 区分 | フィード(到達確認済み) |
| --- | --- |
| 国内 IT メディア | ITmedia AI+、Publickey、CodeZine、gihyo.jp、PC Watch |
...
- ページ見出しはヒーローカードを使わず、他ページと同じセクション見出しに揃える

**掲載日の導線**

掲載日は日々増え続けるため、全件を列挙するセレクトは採用しない。

- 「←」「→」で前後の掲載日へ移動する(存在しない側はボタンを無効化)
- 日付入力(`<input type="date">`)で任意の日付へ移動する
...
```text
┌─────────────────────────────────────┐
│ OpenAIが新モデルGPT-5.5を  ITmedia AI+ │
│ 発表                    ・2026年7月30日 │
│                          元記事を読む → │
│                                       │
│ 推論時間スケーリングを可変にできる新     │
│ ────────────                          │
│ 方式を採用し、従来比でコストを大幅に     │
│ 削減しました。API は同日から一般提供が   │
│ 始まり、既存モデルは 3 か月後に提供終了  │
│ となります。                           │
│                                       │
│ ╭──────────────────╮                │
│ │ 💼 業務での使いどころ │ ▾              │
...
- 見出し(平易な日本語)と、その右に出典名・記事公開日・元記事リンクをまとめて配置する
- **要点まとめ**(散文 120〜180 字)。箇条書きは使わない。件数を固定すると情報が落ち、行数も増えるため
- 用語注は本文中の該当語に点線の下線を引き、**ホバー / タップ / キーボードフォーカス**で説明を表示する(独立した用語枠は設けない)
- 観点に応じた見出し(§5.3)は**チップ(角丸タグ)**として表示し、それ自体を折りたたみのトグルとする。説明文は既定で折りたたむ
- 👍 / 👎 ボタンと合計件数はカード下部の左に置く

順位バッジ、カテゴリ、タグはカードに表示しない。順位は並び順が示し、カテゴリは観点別見出しと重複し、タグは学習用データで閲覧時には不要なため(データとしては保持する)。
...
```text
[[推論時間スケーリング]]を可変にできる新方式を採用し、従来比でコストを…
```
...
- 冒頭に「今週の AI 動向」の要約(3〜5 文)
- 続けて評価上位 5〜8 件を記事カード形式で再掲(順位は週次時点の `final_score` 順で固定)
- 会議で画面共有する前提のため、1 画面に収まりすぎない縦スクロール構成を許容する

### 8.4 空状態・エラー

- 当日分が未生成の場合は「本日のニュースはまだ公開されていません」と直近の掲載日への導線を表示する
...
| メソッド | パス | 概要 |
| --- | --- | --- |
| GET | `/api/news` | `?date=YYYY-MM-DD` の日次記事一覧。`final_score` 降順。自分の投票状態と前後の掲載日(`prevDate` / `nextDate`)を含む。指定日に掲載がなければ直前の掲載日に解決し、省略時は最新 |
