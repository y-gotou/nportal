...
- ステータス: 実装完了・運用中(日次・週次 routine 稼働)
...
- 日次: 前回実行時刻以降に公開された記事
- 週次: 掲載日が「実行日の 7 日前(前週木曜)以上、実行日より前」の掲載記事(再収集は行わない)。週次は日次より先に実行されるため木曜の掲載分は窓に入らず、翌週のダイジェストが扱う
...
- URL が異なる同一話題の再掲(日次をまたぐ二重掲載)は選定段階で落とす。新たな事実を含む続報のみ掲載可とする。補助として、収集スクリプトが掲載済み記事とのタイトル類似(CJK バイグラム + 英数トークンの overlap coefficient、閾値 0.4)を検出し、候補に `similar_recent` として注意喚起を付与する。判定は自動排除ではなく、最終判断は LLM が行う
...
- 週次は `final_score` 上位から選ぶが、**practice / learning の最低枠 1 件**を設ける(観点別のスコア目安レンジの差により、これらの観点が構造的に選外となるのを防ぐ)。選定に 1 件も含まれない場合は最下位 1 件を practice / learning の `final_score` 最上位と入れ替える。窓内に該当記事がない場合は適用しない。
- 実行失敗時は当日分が欠けるだけで、翌営業日の実行に影響しない(掲載済み URL の除外により重複は起きない)。週次の選定窓は実行日基準のため、木曜に実行できなかった週次は原則同日中に手動で再実行する。
...
- 大見出し「今週の{件数}つの動き」
- 「今週の AI 動向」の要約本文(3〜6 文)。本文中の改行は段落の区切りとして扱い、段落ごとに `<p>` で組む。観点コード名(`tooling` 等)は本文に出さず、画面の日本語ラベルか平易な言い換えを使う(検証スクリプトで機械チェックする)
...
- 週次は選定ルールの逸脱もサーバー側で検証する。解決できた記事の掲載日が選定窓(§4.2)の外にある場合、および窓内に practice / learning の記事が存在するのに選定に 1 件も含まれない場合(§6 の最低枠)は 400 とする
...
| — ✅ | **`main` merge 後に本番 D1 へ `npm run db:schema:prod` を実行する** | 完了(2026-07-31 に適用。`news_*` の 3 テーブルが本番に存在) |
...
| 4 ✅ | クラウドタスク(日次)の作成と定期実行 | 完了(2026-07-31 に初回実行、2026-08-03 に日次スケジュールを確定し稼働中) |
| 5 ✅ | 週次ダイジェストの生成とタブ表示 | 完了(2026-08-06 に初回の週次実行を確認し、選定・概況文の品質改善を実施) |
