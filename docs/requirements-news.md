...
- 順位バッジ、カテゴリ、記事公開日
- 見出し(平易な日本語)
- 箇条書き要約 3 点(各 40 文字程度)
- 観点に応じた見出し(§5.3)と説明文 1〜2 文
- タグ、出典名、元記事リンク(別タブで開く)
- 👍 / 👎 ボタンと合計件数

専門用語には短い語句説明を付ける。表示方法(ホバー / タップ)は実装時に UI 検討する。
...
| メソッド | パス | 概要 |
| --- | --- | --- |
| GET | `/api/news` | `?date=YYYY-MM-DD` の日次記事一覧。省略時は最新掲載日。`final_score` 降順。自分の投票状態を含む |
| GET | `/api/news/weekly` | `?date=` の週次ダイジェスト(overview + 記事一覧)。省略時は最新 |
| GET | `/api/news/dates` | 掲載日一覧(日付セレクト用) |
