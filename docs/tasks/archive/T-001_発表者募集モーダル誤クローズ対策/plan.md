...
- [x] 1. dirty 判定純関数とユニットテストを追加する
- [x] 2. speakers.vue の閉じ処理を requestCloseForm 方式に変更する
- [x] 3. docs/requirements-speakers.md を新規作成する(仕様書反映)

## 検証方法

### エージェント実施
- [x] `npm test` — 新規ユニットテストを含め通過
- [x] `npm run check` — 型チェック・本番ビルド通過
- [x] ローカルブラウザ確認(`npm run dev` + モックログイン、Playwright 経由) — 受入条件の3シナリオ: 未入力→外側クリックで即閉じ / 入力あり→外側クリック・キャンセルで確認表示、キャンセルで入力保持・OK で閉じる / 編集モーダルは読み込み値からの変更時のみ確認
